function hierarchicalLatentSpaceHandler(src, eventdata, action, ...
                                        visualiseNodes, nodeIndex, childIndex)
...
if ~isempty(axesIndex)
  switch action
   case 'toggleChildUpdateDynamics'
    visualiseInfo(nodeIndex).tree.updateChild(childIndex) = get(visualiseInfo(nodeIndex).updateChildrenRadio(childIndex), 'value');
    disp([childIndex, nodeIndex])
   case 'toggleDynamics'
    visualiseInfo(axesIndex).runDynamics = ~visualiseInfo(axesIndex).runDynamics;
    set(visualiseInfo(axesIndex).dynamicsRadio, 'value', visualiseInfo(axesIndex).runDynamics);
    
   case 'dynamicsSliderChange'
    X = modelOut(visualiseInfo(axesIndex).model.dynamics, get(visualiseInfo(axesIndex).dynamicsSlider, 'value'));
    x = X(1);
    y = X(2);
